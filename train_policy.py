...
class DiffusionPolicyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = {"pixels": pixels, "wrist_pixels": wrist_pixels, self.action_key: action,}
        ...
        return {
            "pixels": data["pixels"],
            "wrist_pixels": data["wrist_pixels"],
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "wrist_obs_encoder_state_dict":
            policy.wrist_obs_encoder.state_dict(),

        "multiview":
            policy.wrist_obs_encoder is not None,

        "wrist_obs_feature_dim":
            policy.wrist_obs_encoder.output_shape()[0],
...
def train_one_epoch(
    ...
    policy.wrist_obs_encoder.train()
    ...
    params = list(
        chain(
            ...
            policy.wrist_obs_encoder.parameters(),
        )
    )
...
def run(cfg):
    ...
    wrist_obs_feature_dim = (
        wrist_obs_encoder.output_shape()[0]
    )

    if wrist_obs_feature_dim != obs_feature_dim:
        raise ValueError(
            "Overhead and wrist feature dimensions differ: "
            f"{obs_feature_dim} vs "
            f"{wrist_obs_feature_dim}"
        )
    ...
    policy.wrist_obs_encoder.to(device)
