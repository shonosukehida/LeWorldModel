...
/* Test Lite6 FK and IK at the same time, IK can return all solutions */
/* ATTENTION: UFACTORY 850 can use the same logic in this demo code, just modify to use the correct API and joint limits */
/* input: joint angles (dimension: DOF) in degree */

/* *************************************************/
/* execution example: ./main_lite6 10 -30 0 100 0 -52  */
/* *************************************************/
...
int main(int argc, char **argv)
{
	...
	double q_min[6] = UFLITE6_ANGLE_MIN; 
	double q_max[6] = UFLITE6_ANGLE_MAX;
	...
	uf_lite6_config(q_max, q_min, tcp_offset); 
	

	/*** Method 1: Use Transformation Matrix as TCP expression */
	int ret = uf_lite6_forward_kinematics(theta, T); // result is in transformation matrix

	fprintf(stderr, "Lite6 FK (Matrix) result:\n" );
	for(int i=0; i<4; i++)
	{
		for(int j=0; j<4; j++)
	...
	ret = uf_lite6_inverse_kinematics(T, q_pre, theta_sol, q_max, q_min, &all_sol);
	...
	ret = uf_lite6_forward_kinematics(theta, pose_rpy);	
	fprintf(stderr, "\nLite6 FK (Pose) result:\n" );
	for(int i=0; i<6; i++)
	{	
	...
	ret = uf_lite6_inverse_kinematics(pose_rpy, q_pre, theta_sol);
